...
function openProcessTab(tab: string, extraQuery: Record<string, unknown> = {}) {
  if (isPlatformRuntimePath(route.path, 'process')) {
    const query = {
    ...
    const resolved = router.resolve({
      path: buildPlatformProcessTabPath(tab),
      query,
    })
  ...
  const targetPath = tab === 'instance' ? '/process/instance' : '/process/modeling'
  const queryParams = withActiveTenantQuery(extraQuery, resolveCurrentActiveTenant())
  ...
  window.open(`${targetPath}?${search.toString()}`, '_blank')
}
...
async function fetchTasks() {
  try {
    ...
    console.log('查询参数:', {
      assignee,
      activeTenantId: resolveCurrentActiveTenant(),
      status: query.status,
    })
...
function viewProcessInstance(task: Task | Record<string, any>) {
  openProcessTab('instance', { instanceId: task.processInstanceId })
}
