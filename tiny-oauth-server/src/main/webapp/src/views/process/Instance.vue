  ...
  <div class="content-container" style="position: relative">
    <div class="content-card">
      <div class="form-container">
        <a-form layout="inline" :model="query">
          ...
          <a-form-item v-if="!isPlatformProcessRuntime" label="记录租户筛选">
            <a-select
              v-model:value="query.recordTenantId"
              placeholder="默认当前活动租户"
              allow-clear
              style="width: 150px"
            >
      ...
      <div class="toolbar-container">
        <div class="table-title">流程实例列表</div>
              ...
              >
                <div style="display: flex; align-items: center">
                  <a-checkbox
                    ...
                    :indeterminate="
                      showColumnKeys.length > 0 && showColumnKeys.length < allColumns.length
                    "
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.dataIndex === 'id'">
                <a-typography-text code class="table-code-text" :title="record.id">
                  {{ record.id }}
                </a-typography-text>
              </template>
              <template v-else-if="column.dataIndex === 'processKey'">
                <a-typography-text code class="table-code-text" :title="record.processKey">
                  {{ record.processKey }}
                </a-typography-text>
              </template>
              <template v-else-if="column.dataIndex === 'processDefinitionId'">
                <a-typography-text
                  code
                  class="table-code-text"
                  :title="record.processDefinitionId"
                >
              ...
              <template v-else-if="column.dataIndex === 'recordTenantId'">
                <a-tag v-if="record.recordTenantId" color="green">{{
                  record.recordTenantId
                }}</a-tag>
              ...
              <template v-else-if="column.dataIndex === 'action'">
                <div class="action-buttons">
                  ...
                  <a-button
                    type="link"
                    size="small"
                    @click.stop="throttledSuspend(record)"
                  ...
                  <a-button
                    type="link"
                    size="small"
                    @click.stop="throttledActivate(record)"
                    v-if="record.state === 'suspended'"
                    class="action-btn"
                  >
                    <template #icon>
                      <PlayCircleOutlined />
                  ...
                  <a-button
                    type="link"
                    size="small"
                    @click.stop="throttledViewTasks(record)"
                    class="action-btn"
                  >
                    <template #icon>
                      <TagsOutlined />
    ...
    <a-modal v-model:open="showInstanceDetail" title="流程实例详情" :footer="null" width="800px">
      <a-descriptions :column="2" bordered v-if="selectedInstance">
        ...
        <a-descriptions-item label="记录所属租户ID">
          <a-tag v-if="selectedInstance.recordTenantId" color="green">{{
            selectedInstance.recordTenantId
          }}</a-tag>
          <a-tag v-else color="default">默认</a-tag>
        </a-descriptions-item>
        <a-descriptions-item label="流程变量" :span="2">
          <a-list
            v-if="selectedInstance.variables && Object.keys(selectedInstance.variables).length > 0"
            :data-source="Object.entries(selectedInstance.variables)"
            size="small"
          >
...
const INITIAL_COLUMNS = [
  { title: 'ID', dataIndex: 'id', sorter: true, width: 260, ellipsis: true },
  { title: '流程定义Key', dataIndex: 'processKey', sorter: true, width: 180, ellipsis: true },
  { title: '流程定义ID', dataIndex: 'processDefinitionId', sorter: true, width: 360, ellipsis: true },
  ...
  { title: '记录所属租户ID', dataIndex: 'recordTenantId', width: 160, ellipsis: true },
  {
    title: '操作',
    dataIndex: 'action',
    width: 300,
...
function resetColumnOrder() {
  ...
  showColumnKeys.value = INITIAL_COLUMNS.filter(
    (col) => typeof col.dataIndex === 'string' && col.dataIndex,
...
function onDragEnd() {
  console.log(
    '拖拽结束，新顺序:',
    draggableColumns.value.map((col) => col.title),
  )
}
...
const throttledSuspend = useThrottle(
  (record: unknown) => handleSuspend(record as ProcessInstance),
  500,
)
...
const throttledActivate = useThrottle(
  (record: unknown) => handleActivate(record as ProcessInstance),
  500,
)
...
const throttledViewTasks = useThrottle(
  (record: unknown) => handleViewTasks(record as ProcessInstance),
  500,
)
...
const throttledDelete = useThrottle(
  (record: unknown) => handleDelete(record as ProcessInstance),
  500,
)
...
const goToModeling = () => {
  router.push({
    path: isPlatformRuntimePath(route.path, 'process')
      ? buildPlatformProcessTabPath('modeling')
      : '/modeling',
