  ...
  <div class="content-container" style="position: relative">
    <div class="content-card">
      ...
      <div class="toolbar-container">
        <div class="table-title">流程定义列表</div>
              ...
              <template v-else-if="column.dataIndex === 'action'">
                <div class="action-buttons">
                  ...
                  <a-button
                    type="link"
                    size="small"
                    @click.stop="throttledPreview(record)"
                    class="action-btn"
                  >
                    <template #icon>
                      <FileImageOutlined />
                  ...
                  <a-button
                    type="link"
                    size="small"
                    @click.stop="throttledStart(record)"
                    class="action-btn"
                  >
                    <template #icon>
                      <PlayCircleOutlined />
                  ...
                  <a-button
                    type="link"
                    size="small"
                    @click.stop="throttledToggleSuspend(record)"
                    class="action-btn"
                  >
                    <template #icon>
    ...
    <ProcessDeployResultModal
      v-model:open="showSaveResult"
      :result="{
        deploymentId: processInfo.deploymentId,
        deploymentName: processInfo.name,
        description: processInfo.description,
      }"
      @go-deployment="goToManagement"
      @go-definition="goToDesigner"
    />
    ...
    >
      <a-descriptions bordered :column="1" size="middle">
        ...
        <a-descriptions-item label="记录所属租户ID">
          <a-tag v-if="viewRecord?.recordTenantId" color="green">{{
            viewRecord?.recordTenantId
          }}</a-tag>
          <a-tag v-else color="default">默认</a-tag>
        </a-descriptions-item>
      ...
      <template #footer>
        <a-space>
          <a-button type="primary" @click="() => viewRecord && throttledPreview(viewRecord)"
            >预览流程</a-button
          >
...
function buildProcessTabQuery(_tab: string) {
  return isPlatformRuntimePath(route.path, 'process')
...
const throttledPreview = useThrottle(
  (record: unknown) => handlePreview(record as ProcessDefinition),
  500,
)
...
const throttledStart = useThrottle(
  (record: unknown) => handleStart(record as ProcessDefinition),
  500,
)
  ...
  Modal.confirm({
    ...
    onOk: () => {
      return deploymentApi
        .deleteDeployment(record.deploymentId)
        .then(() => {
...
const throttledDelete = useThrottle(
  (record: unknown) => handleDelete(record as ProcessDefinition),
  500,
)
...
const throttledToggleSuspend = useThrottle(
  (record: unknown) => handleToggleSuspend(record as ProcessDefinition),
  500,
)
...
const goToDesigner = () => {
  router.push({
    path: isPlatformRuntimePath(route.path, 'process')
      ...
      : '/workflowDesign',
    query: buildProcessTabQuery('modeling'),
  })
}
...
const goToManagement = () => {
  ...
  router.push({
    path: isPlatformRuntimePath(route.path, 'process')
      ...
      : '/deployment',
    query: buildProcessTabQuery('deployment'),
  })
}
