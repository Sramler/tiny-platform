      ...
      <div class="table-scroll-container">
        ...
        <a-table :columns="columns" :data-source="tableData" :pagination="false"
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column && column.dataIndex === 'builtinLocked'">
              <a-tag :color="record.builtinLocked ? 'orange' : 'default'">
                {{ record.builtinLocked ? '已锁定' : '未锁定' }}
              </a-tag>
            </template>
            <template v-else-if="column && column.dataIndex === 'dictCode'">
              <a-button type="link" @click.stop="handleViewItems(record)" class="dict-code-link"
            ...
            <template v-else-if="column && column.dataIndex === 'action'">
              <div class="action-buttons">
                ...
                <a-button type="link" size="small" @click.stop="handleViewItems(record)" class="action-btn">
                  <template #icon>
                    <EyeOutlined />
                  </template>
                  查看字典项
    ...
    <a-drawer v-model:open="drawerVisible"
      ...
      <a-form :model="formState" layout="vertical">
        ...
        <a-form-item label="是否内置" name="isBuiltin">
          ...
          <span style="margin-left: 8px; color: #999; font-size: 12px;">平台内置字典，创建后不可修改</span>
        </a-form-item>
        <a-form-item label="是否锁定" name="builtinLocked" v-if="formState.isBuiltin">
          <a-switch v-model:checked="formState.builtinLocked"
            :disabled="drawerMode === 'view' || drawerMode === 'edit'" />
          <span style="margin-left: 8px; color: #999; font-size: 12px;">锁定后不允许删除和修改关键字段</span>
        </a-form-item>
...
const emit = defineEmits<{
  (e: 'viewItems', dictTypeId: number): void
}>()
...
async function loadData() {
  ...
  try {
    ...
    message.error('加载字典类型数据失败: ' + (error?.message || '未知错误'))
  } finally {
    loading.value = false
  }
}
