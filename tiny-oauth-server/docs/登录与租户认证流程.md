...
10. “待补全 MFA” 会话使用 `MultiFactorAuthenticationToken`，并保持 `authenticated=true`；是否继续 challenge 由是否缺少 `TOTP` factor 决定。  
...
| 端点 | 允许的认证状态 | 补充约束 |
|---|---|---|
| `/self/security/status` | challenge / full | 仅查看状态 |
| `/self/security/totp-bind` `/self/security/totp-verify` | challenge / full | 仅页面 challenge |
| `/self/security/totp/bind-form` `/self/security/totp/check-form` | challenge / full | 需携带 CSRF |
| `/self/security/totp/skip` `/self/security/skip-mfa-remind` | challenge / full | 仅未激活 TOTP、非 `REQUIRED`、非 `NONE` 时可用 |
...
sequenceDiagram
  ...
  participant M as AuthorizationEndpointMfaGate
  ...
  alt requireTotp=true 且未完成 TOTP
    H-->>L: 返回 challenge token(authenticated=true, FACTOR_PASSWORD)
...
5. challenge MFA 会话应满足：  
   - `authenticated=true`  
   - 具备 `FACTOR_PASSWORD` 且缺少 `FACTOR_TOTP`  
