-- ========================================================================
-- 调度任务示例数据（可选）
-- 依赖：已执行 data.sql 调度段或 scheduling-demo-data.sql，且存在 tenant_id=1。
-- 用途：导入「数据导出流水线」「报表计算流水线」「健康检查」示例，便于演示。
-- 使用：在目标库执行本脚本；若主键冲突可调整下方 ID 或先删除再插。
-- ========================================================================

-- 示例一：数据导出流水线（任务类型 + 3 任务 + DAG + 版本 + 3 节点 + 2 边）
...
-- 示例二：报表计算流水线（2 任务类型 + 3 任务 + DAG + 版本 + 3 节点 + 2 边，并行→汇总）
INSERT IGNORE INTO `scheduling_task_type`
(`id`, `tenant_id`, `code`, `name`, `description`, `executor`, `default_timeout_sec`, `default_max_retry`, `enabled`, `created_at`, `updated_at`)
VALUES
...
-- 示例三：健康检查（1 任务类型 + 2 任务 + DAG + 版本 + 2 节点 + 1 边）
INSERT IGNORE INTO `scheduling_task_type`
(`id`, `tenant_id`, `code`, `name`, `description`, `executor`, `default_timeout_sec`, `default_max_retry`, `enabled`, `created_at`, `updated_at`)
VALUES
