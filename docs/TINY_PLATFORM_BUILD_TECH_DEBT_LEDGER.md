...
| Vite | 入口 `index-*.js` 过大（历史 ~2.8MB 单文件） | **已推进**：业务路由使用异步 chunk，依赖按 Vue/Antd/Axios/BPMN 等拆分；2026-08 Web 认证固定为 Session 后已删除 OIDC browser chunk、callback 与调试页。可用 `npm run build:analyze` 复核，`build-only` 必须无 mixed import 告警。 |
