...
- ✅ **展示映射与业务分类**：管理稳定编码的展示元数据，以及可按规则维护的业务分类
...
字典中心只承载“展示可治理的稳定编码映射”和“可配置业务分类”，不得吞掉枚举定义、主数据、配置中心和业务模型本身。

生产级 SaaS 中，不能把所有“编码 + 名称”的结构都收敛成一张万能字典表。Tiny Platform 按以下 5 层划分职责：

| 层级 | 归属 | 字典中心职责 | 示例 |
| --- | --- | --- | --- |
| 业务事实字段 | 领域模型 / API 契约 | 不承载字段定义，不动态改名或改变语义 | `latestDesignVersion`、`recordTenantId`、`hasUndeployedChanges` |
| 状态码 / 枚举码 | 后端枚举、前端类型、数据库约束或 seed | 只管理 label / color / sort / i18n 等展示元数据，不管理业务判断 | `workflow_model_status`、`workflow_runtime_state` |
| 业务字典 | 平台或租户可治理的业务分类 | 按 scope、权限、审批、审计规则维护分类项 | 客户等级、行业分类、费用类型 |
| 主数据 | 独立领域模型、独立生命周期、独立 API | 不替代主数据 API；最多引用主数据编码做展示辅助 | 组织、角色、用户组、表单定义、服务任务实现 |
| 系统配置 / 元数据 | 配置中心、系统元模型、插件元数据 | 不混入普通字典表，不通过字典变更系统行为 | 功能开关、租户配额、插件启停、BPMN 固定元素类型 |

关键状态机不得做成纯动态字典。凡会影响后端流程分支、权限判定、部署关系、审计语义或数据一致性的状态值，必须由代码枚举和领域契约固定；字典最多提供展示映射。例如 `workflow_model_status`、`workflow_runtime_state` 会参与工作流模型生命周期和运行态判断，业务判断必须基于稳定枚举值，不能基于字典 label 或租户可编辑项。

SaaS 字典必须显式区分 scope：

- **平台系统字典**：由平台 seed / 升级脚本维护，普通租户不可改编码、删除或改变业务语义。
- **租户业务字典**：租户可在平台允许的范围内维护分类项或覆盖展示 label，但不得突破平台定义的 value 约束。
- **平台发布、租户引用的主数据**：应建模为主数据或资源 API，通过引用关系暴露给租户，不应退化为普通字典项。

判断一个结构是否应进入字典中心时，默认使用以下裁决：

- 如果改变 value 会改变后端逻辑、状态机或权限结果，它不是普通动态字典。
- 如果对象有独立增删改查、审批、版本、归属、生命周期或复杂属性，它应是主数据或业务模型。
- 如果配置会改变系统行为、插件装载、配额或功能开关，它应进入配置中心或元数据模型。
- 如果只是稳定编码的展示翻译、排序、颜色、国际化，或租户可治理的轻量业务分类，才适合进入字典中心。
