...
- 配套文档：`docs/TINY_PLATFORM_AUTHORIZATION_DOC_MAP.md`（阅读入口与冲突裁决）、`docs/TINY_PLATFORM_PERMISSION_IDENTIFIER_SPEC.md`（权限码规范）、`docs/TINY_PLATFORM_AUTHORIZATION_MODEL.md`（授权模型主线）、`docs/TINY_PLATFORM_AUTHORIZATION_TASK_LIST.md`（当前完成度）
...
- ✅ 运行态功能权限真相源已统一为 `role_permission -> permission`；`resource.permission` 仅保留兼容字段、运营可读字段和历史回填输入，新增逻辑不得再把它当唯一真相源。
...
- 若争议不是“权限码格式/命名”，而是“当前模型是否已落地、carrier/runtime 是否已闭合、`api_endpoint` 证据等级到哪一档”，应按 `docs/TINY_PLATFORM_AUTHORIZATION_DOC_MAP.md` 回到 `docs/TINY_PLATFORM_AUTHORIZATION_TASK_LIST.md`、`docs/TINY_PLATFORM_AUTHORIZATION_LAYERED_MODEL.md`、`docs/TINY_PLATFORM_API_ENDPOINT_GUARD_COVERAGE.md` 与 `93-tiny-platform-authorization-model.rules.md` 裁决。
