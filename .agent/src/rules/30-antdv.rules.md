...
- ✅ 对树表懒加载链路，如果后端用 `leaf=false` 表示“仍有子节点可加载”，即使本次响应里的 `children` 是空数组，前端也不能把它当成终态叶子节点；必须保留可展开语义、在 `@expand` 时按需请求直子节点，并补浏览器/组件契约回归测试锁住该行为。

### 4) Tabs 规范

- ✅ `a-tabs` 的当前页签如果是页面级 / 路由级状态，默认使用 path 子路由表达，例如 `/platform/process/definition`、`/platform/dicts/type`；不要再用 `?tab=` 作为主导航状态。
- ✅ 路由驱动的 `a-tabs` 必须使用 `:active-key` + `@change` 更新路由，让路由成为唯一真相源；不要再额外维护一份本地 `ref + watch` 来同步同一个 tab 状态。
- ✅ 路由驱动的重型页签内容应使用 `destroy-inactive-tab-pane` 或等价按需挂载策略，避免隐藏页签提前发请求、注册定时器或污染当前页上下文。
- ✅ 不得通过上层 `router-view :key` 拼接业务 `tab` query 来修复页签切换问题；应在使用 `a-tabs` 的业务组件内处理受控状态、按需挂载和 path 子路由回写。
- ✅ 涉及 deep-link / 刷新进入的 tabs 页面，必须覆盖“子路由直开 / 刷新能渲染正确页签”和“点击页签会更新到对应 path 且清理不应透传的上下文参数”两类回归。
- ✅ 只有纯展示偏好、筛选条件或临时 UI 状态才可考虑 query 参数；若确需使用 `query.tab`，必须在同一变更中说明为什么不能建模为 path，并补路由回归测试锁住。
