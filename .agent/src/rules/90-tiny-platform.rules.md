...
- ❌ 默认把 `main` 当作 tiny-platform 日常开发主线。
- ❌ 默认把整条 `sb4` 同步到 `sb3`，或用整线 merge 代替维护回补。
...
- ✅ 需要进入 `sb3` 的修复，必须先在 `sb4` 收敛，再通过 `cherry-pick -x` 或专用 backport 分支选择性回补；不得默认创建整线 `sb4 -> sb3` 同步 PR。
- ✅ 调整分支治理、AI 执行边界、PR 模板或自动化 workflow 时，必须在同一变更中同步更新 `AGENTS.md`、`.agent/src/rules/**`、相关文档与 workflow，防止 Cursor / Codex / CI 读取到分裂规则。
...
- ✅ Camunda 7 在 tiny-platform 中默认保持 Engine Only；原生 Camunda REST 必须通过显式 `camunda-rest` 构建 profile 才能进入运行时制品，不得因 starter 在 classpath 上而隐式暴露。Camunda REST 的 Jackson 3 升级必须同时清除 `spring-boot-jackson2`、`jersey-media-json-jackson` 以及 Jackson 2 core/databind/dataformat/datatype（Jackson 3 官方 `com.fasterxml.jackson.annotation` 不在禁止范围），并完成真实 HTTP JSON 往返验证；Spin 迁移必须单独验收变量序列化兼容性。
- ✅ Camunda fork 制品不得覆盖既有版本；REST/Jackson/Spin 兼容边界发生变化时必须发布新的不可变 fork 版本。tiny-platform 切换版本前必须依次通过 fork REST 随机端口测试、Spin 完整模块测试、默认/REST 依赖树门禁，以及 `-Pcamunda-rest` 下真实 MySQL 应用级 E2E；条件跳过不能记为通过，E2E 结束后必须校验部署、流程定义和运行实例无残留。
- ✅ 一方代码不得依赖可选集成 starter 的传递依赖获得基础 JSON API；Spring Boot 4 一方实现与测试统一使用 `tools.jackson.core` / `tools.jackson.databind`，仅 Jackson annotations 继续使用官方 `com.fasterxml.jackson.annotation` 命名空间。移除 starter 后必须同时跑主代码编译、测试编译、受影响行为测试和运行时依赖树门禁。
