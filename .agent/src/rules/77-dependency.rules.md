...
- ❌ 将开发者本地 `mvn install` / 本地缓存 / 未发布产物作为 consumer 能否解析内部 fork 依赖的唯一依据。
- ❌ 同一内部 fork 依赖在不同分支或不同模块中混用“本地 install 版、SNAPSHOT 版、固定发布版”而不留痕。
- ❌ 长期依赖无退出计划的 SNAPSHOT fork 版本进入稳定交付线。
...
- ✅ 引入内部 fork 依赖时，必须记录其来源仓库、fork 仓库、发布仓库或 fallback 解析方式。
- ✅ 内部 fork 依赖优先使用可重复解析的固定版本；SNAPSHOT 只允许作为过渡状态，并必须写清退出条件。
- ✅ 变更内部 fork 坐标、版本或发布策略时，必须在同一变更中验证至少一条 consumer 构建或启动路径。
- ✅ 若 `sb3` 与 `sb4` 使用不同的依赖基线或 fork 版本，必须在文档或提交说明中明确其适用分支，避免双线漂移失控。
...
- ⚠️ 重大升级（如 Spring Boot、Spring Security、Jackson、Camunda fork）建议维护一份分支化的升级清单，注明 `sb3`/`sb4` 的差异和收口状态。
