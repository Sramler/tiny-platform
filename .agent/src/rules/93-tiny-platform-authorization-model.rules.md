...
- ❌ 遇到 `api_endpoint requirement denied`、`URI_TEMPLATE_NOT_MATCHED` 或页面跳转 403 时，通过恢复浏览器 Bearer token、扩大过滤器豁免、绑定更大权限或改前端路由守卫来掩盖载体缺失。
...
- ✅ 权限载体必须按“页面业务闭包”治理：盘点页面进入、布局/bootstrap、列表/统计/历史/详情、弹窗预载，以及 create/update/delete/trigger/retry/stop/pause/resume 等全部并发和延迟请求；每个 HTTP method + 精确 URI template 都必须有 `api_endpoint`、requirement、初始化/回填与允许/拒绝测试，不能只补首个报错接口。
- ✅ `api_endpoint` 403 必须先按守卫证据分类：`URI_TEMPLATE_NOT_MATCHED` 或 `candidateCount=0` 表示 method/template 载体缺失或漂移；载体已匹配但 requirement 不满足才表示当前身份授权不足。前者修载体/初始化，后者修授权或产品权限设计，禁止把两类问题统称为“token 问题”。
- ✅ 前端路由进入成功不代表页面授权闭合；任一并行、后台轮询、modal preload 或懒加载 API 的 403 都可能触发全局 `/exception/403`。修复时必须结合浏览器失败请求、后端同一 `traceId` 日志和页面接口清单定位实际拒绝端点。
- ✅ 权限初始化必须按依赖顺序执行并验证非零结果：先建立 permission，再建立全局 endpoint template/requirement，再为现存租户克隆/回填 permission 与 carrier，最后绑定 role；late-created tenant 必须走同一模板链。引用尚不存在 permission 的合法 SQL 产生 0 行也视为迁移失败。
- ✅ 通配权限与具体权限是两个独立标识，除非统一 matcher 有已测试的显式蕴含规则；不得假设角色持有 `scheduling:*` 就自动满足 `scheduling:console:view`，seed、requirement 与角色绑定必须使用同一规范码语义。
- ✅ 已执行的 Liquibase changeset 不得改写以补 endpoint/permission；必须追加幂等补偿 changeset，并同时验证 fresh DB、已执行库升级、现有租户回填和新租户克隆，避免 checksum 漂移与“新库绿、旧库仍 403”。
- ✅ `menu / ui_action / api_endpoint` 拆分载体迁移必须按依赖拓扑执行：先建立目标 scope 的 permission 与 menu，再复制/创建 ui_action，最后写 requirement 与 role_permission；凡通过 `JOIN target_menu` 复制按钮的 changeset，fresh DB 必须断言目标页面菜单、按钮及 requirement 行数均大于 0，并以真实浏览器验证动态路由已注册且页面业务接口确实发出。不得把“SQL 执行成功但 JOIN 结果为 0”视为载体闭包完成。
- ✅ 聚合 `menu / ui_action / api_endpoint` 为资源树时必须把载体类型视为节点身份的一部分：三个表的主键序列彼此独立，ID 相同不代表父子关系；只有 `directory/menu` 可以递归查询子节点，`ui_action/api_endpoint` 必须是叶子，并对菜单祖先环 fail-safe 截断。必须包含“按钮 ID 等于父菜单 ID”的回归样例，禁止只按裸 ID 跨载体递归。
