...
- ✅ tiny-platform 正式维护两条交付分支：`sb3`（Spring Boot 3 稳定线）与 `sb4`（Spring Boot 4 升级线）。
- ✅ `main`/`master` 视为历史基线或默认入口，不作为日常修复、升级改造、常规 PR 的默认目标分支，除非用户明确要求。
- ✅ 通用业务修复、安全修复、需要双线存在的改动优先进入 `sb3`，验证通过后再用 `cherry-pick` 或等价方式同步到 `sb4`。
...
- ⚠️ PR 目标分支必须与变更性质一致：稳定修复走 `sb3`，升级改造走 `sb4`，不要例行提交到 `main`/`master`。
...
- `sb3`：当前稳定交付线，承载业务修复、线上问题修复、保守演进。
- `sb4`：升级演进线，承载 Spring Boot 4、依赖升级、兼容修复、框架迁移。
...
- ✅ 需要双线存在的改动应遵循 `sb3 -> sb4` 的同步顺序，不能默认把 `sb4` 反向合回 `sb3`。
