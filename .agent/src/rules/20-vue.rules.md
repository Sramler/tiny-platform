...
- ❌ 在同一个 async Vue Router guard 中混用 `next(...)` 与 return 风格，导致 `Invalid navigation guard`、重复跳转或导航中断。
- ❌ 把全局 catch-all 404 直接写成无条件 redirect，导致动态菜单/动态路由页面在首次直刷或 deep-link 时，还没等路由注入就被提前吞掉。
- ❌ 只验证“从菜单点击进入页面”这一条路径，而不验证同一页面的 direct deep-link / 浏览器刷新路径。
...
- ✅ 修改路由守卫、动态菜单路由注入、菜单驱动页面入口时，必须至少覆盖两条入口：菜单点击路径与 direct deep-link / 刷新路径；不能只保一条。
- ✅ 动态路由页面的守卫修复必须锁住“首次解析未命中 -> 菜单路由加载 -> 重新 resolve / replace 成功”的回归测试，避免以后再被 404 redirect 抢先吞掉。
...
- ⚠️ 对菜单驱动、懒加载、动态路由页面，建议同时保留“router/guard 契约测试 + 真实浏览器验证”，避免只靠其一判断通过。
