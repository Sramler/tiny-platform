...
- ✅ 涉及分支模型、升级路线、内部 fork 策略、过渡兼容方案、发布策略的变更，必须在同一变更中同步更新对应文档（如 ADR、升级清单、分支治理、发布说明）。
- ✅ 临时过渡方案文档必须写清：背景问题、适用范围、非目标、风险、退出条件，以及后续收口方向。
- ✅ 当同一方案仅适用于 `sb3` 或 `sb4` 某一条分支时，文档中必须明确标注适用分支，避免双线混淆。
- ✅ 涉及 tiny-platform 默认开发主干、维护线或 backport 边界切换时，必须在同一变更中同步更新 `AGENTS.md`、`.agent/src/**` 规则源、PR 模板与相关 workflow，避免 AI 工具和人工文档读到不同口径。
- ✅ 分支治理文档一旦改口径，历史任务卡或盘点文档若仍保留旧表述，必须显式标注“历史快照”或给出当前态跳转入口，避免 Cursor / Codex 被旧文档误导。
- ✅ 任务清单从“待完成”切换为“已完成”时，必须同步清理同一文档中的旧完成条件、待复验描述和相反状态；禁止 checklist 已勾选但正文仍写“仅待 CI 后收口”。
- ✅ 测试/流水线证据记录必须区分代码承载提交、文档收口提交、手动 workflow、自动 workflow 与 path-filter 未触发项，并记录精确 SHA、通过数量和可追溯 run 链接或 run id；不得用模糊的“本次提交”“全部流水线”掩盖证据实际对应的提交。
...
- ⚠️ 升级类与治理类文档建议同步记录关键验证证据，例如执行命令、通过日期、适用分支、已完成/待收口状态。
- ⚠️ 对内部 fork 或重大依赖升级，建议维护“决策文档 + 操作清单 + 证据报告”三件套，避免只有结论没有落地依据。
